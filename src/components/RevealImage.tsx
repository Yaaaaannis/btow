...
declare module "@react-three/fiber" {
  interface ThreeElements {
    imageRevealMaterial: any;
  }
}

type ImageRevealMaterialType = {
...
const RevealImage = ({
  ...
  const materialRef = useRef<ImageRevealMaterialType>(null);
  const loadedTexture = typeof texture === 'string' ? useTexture(texture) : texture;
  const hasAnimated = useRef(false);

  useEffect(() => {
    if (isClicked && !hasAnimated.current) {
  ...
  }, [isClicked, revealProgress, onRevealComplete]);
  ...
  return (
    <mesh>
      ...
      <imageRevealMaterial ref={materialRef} />
    </mesh>
  );
};
